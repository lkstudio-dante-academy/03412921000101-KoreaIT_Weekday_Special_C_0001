...
int E01Example_04(int argc, char* args[])
{
	 ...
	 *
	 * 위와 같이 nResult 변수에는 0.5 가 아닌 0 이 할당 된다. (+ 즉, / 연산자의 피연산자가
	 ...
	 *
	 * 위와 같이 fResult 변수에는 0.5 가 할당 된다. (+ 즉, 정수는 소수점을 표현하는 것이
	 ...
	 *
	 * 위와 같이 fResult 변수에는 0.5 가 할당 된다. (+ 즉, 형변환 연산자에 의해 nValB 가
	 ...
	 *
	 * C 은 컴퓨터와 마찬가지로 0 을 거짓으로 인식하고 0 을 제외한 모든 수를 참으로
	 * 인식한다.
	 */
	printf("\n=====> 관계 연산자 <=====\n");
	printf("%d < %d = %d\n", nValA, nValB, nValA < nValB);
