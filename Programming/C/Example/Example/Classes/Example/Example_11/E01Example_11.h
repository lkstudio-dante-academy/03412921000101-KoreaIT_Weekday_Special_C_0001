...
* 알 수 있다.
* 
* 또한 함수의 입력은 필요에 따라 원하는 만큼 데이터를 전달하는 것이 가능하지만 출력은
* 하나의 데이터만 출력하는 것이 가능하다. (+ 즉, C 에서 함수가 2 개 이상의 데이터를
* 반환 할려면 구조체 등을 활용해야한다.)
* 
* C 함수 선언 방법
* - 반환형 + 함수 이름 + 매개 변수
* 
...
* printf("Hello, World!\n");
* 
* 위와 같이 printf 함수를 활용하면 콘솔 창에 문자열을 출력하기 위한 명령문의 중복을 
* 최소화 시키는 것이 가능하다.
* 
* 또한 printf 함수의 사용법만 알고 있다면 printf 함수 내부에 작성 된 명령문을 몰라도 해당 명령문을
* 재사용하는 것이 가능하다. (+ 즉, 완성 된 함수는 사용법만 알고 있으면 해당 함수 내부에 작성 된
* 명령문을 언제든지 재사용하는 것이 가능하다.)
*/
