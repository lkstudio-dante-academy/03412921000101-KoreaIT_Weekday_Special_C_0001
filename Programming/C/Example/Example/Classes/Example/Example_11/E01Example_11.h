...
/*
* 함수 (Function) 란?
* - C/C++ 로 작성 된 명령문 일부 or 전체를 따로 떼어내서 재사용 할 수 있는 기능을 의미한다. (+ 즉,
* 함수를 활용하면 중복적으로 발생하는 명령문을 최소화 시키는 것이 가능하다.)
* 
* C/C++ 의 함수는 수학의 함수와 달리 입력과 출력이 존재하거나 없을 수 있다. (+ 즉, 
* C/C++ 의 함수는 입력과 출력이 필수가 아니라는 것을 의미한다.)
* 
* C/C++ 함수 유형
...
* 
* 위와 같이 C/C++ 의 함수는 입력과 출력의 존재 여부에 따라 4 가지 유형이 존재한다는 것을 
* 알 수 있다.
* 
* C/C++ 함수 선언 방법
* - 반환형 + 함수 이름 + 매개 변수
...
* 함수가 호출되지 않으면 함수 내부에 작성 된 명령문은 실행 되지 않는다는 것을 의미한다.)
* 
* 함수 선언 vs 함수 구현
* - 함수 선언은 함수의 존재 여부를 C/C++ 컴파일러에게 알리는 역할을 수행한다. (+ 즉,
* 함수 선언은 단순히 특정 함수가 존재한다는 사실을 C/C++ 컴파일러에게 알리는 것이기 때문에
* 중복을 허용한다는 것을 알 수 있다.)
* 
...
* 
* C/C++ 컴파일러는 위에서 아래 방향으로 명령문을 해석하기 때문에 특정 함수를 호출하기 전에
...
* (+ 즉, 함수 선언을 파일 상단에 작성함으로서 C/C++ 컴파일러에게 함수의 존재 여부를 알린다는 것을
* 의미한다.)
* 
* 함수를 활용하는 주요 이유
* - 명령문의 중복 최소화
