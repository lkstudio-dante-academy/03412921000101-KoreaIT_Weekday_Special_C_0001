...
*
* 트리 (Tree) 란?
* - 데이터를 계층적인 구조로 관리하는 자료구조를 의미한다. (+ 즉, 트리에 의해서 관리 되는
* 데이터는 상/하 관계를 형성 한다는 것을 알 수 있다.)
*
* 트리는 노드를 통해 데이터를 관리하며 자식 노드의 개수에 따라 N 차 트리 라고 한다.
*
* 프로그래밍에서 주로 활용되는 트리는 자식이 최대 2 개인 이진 트리 (Binary Tree) 이며
* 이진 트리 구조를 활용하면 데이터를 빠르게 탐색하는 이진 탐색 트리 (Binary Search Tree) 를
* 구현하는 것이 가능하다.
*
* 트리 관련 주요 용어
* - 노드					<- 트리의 구성 요소
...
* - 이진 탐색을 수행 할 수 있는 트리를 의미한다. (+ 즉, 이진 탐색 트리는 데이터를
* 빠르게 탐색하는 것이 가능하다.)
*
...
* 위치 시킴으로서 탐색을 빠르게 수행하는 것이 가능하다. (+ 즉, 이진 탐색 트리에서 특정 노드에 있는
* 데이터는 항상 왼쪽 서브 트리와 오른쪽 서브 트리의 중간 데이터가 있다는 것을 알 수 있다.)
*
* 트리 순회 (Tree Traversal) 란?
* - 트리에 존재하는 모든 노드를 방문하는 것을 의미한다. (+ 즉, 순회를 활용하면 트리에 존재하는
* 모든 노드에 접근하는 것이 가능하다.)
*
* 트리 순회 종류
...
/** Example 28 */
int E01Example_28(int argc, char* args[]);
